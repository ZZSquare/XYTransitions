...
def Hamiltonianm(J, Bx, Bzarray):
    ...
    for bz in Bzarray:
        eigenvalue, eigenvector = eigh(XYHamiltonian(XY, B_x, bz))
        #print(eigenvalue)
        #print(eigenvector)
        psi = np.asarray(eigenvector[0])
        #print(psi)
        #psi = np.matmul(XYHamiltonian(XY, B_x, bz), psi0)
        #print(psi)
        #print(eigenvalue)
        #print(eigenvector)
        #m = np.matmul(psi.getH(), np.matmul(BzMatrix(1), psi))
        #m = np.matmul(np.conj(psi), np.matmul(BzMatrix(1), psi))
