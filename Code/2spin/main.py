...
from qiskit import Aer, QuantumRegister, QuantumCircuit, execute, IBMQ, ClassicalRegister, transpile, assemble
from qfunctions2spin import adiabaticramp, theta, twospin_instruction, exactm, interactionm, Hamiltonianm
...
def twospin_df(J, Bx, dB, Bz_max, dt_steps, dt_steps_bool, gamma, skip, provider, backend, pq0, pq1):
    ...
    #while (i < 6):
        ...
        twospin_instruction(circ, J, Bx, thetaarray[:i], dt)
    ...
    if dt_steps_bool == 'dt':
        ...
        calibrated_df.to_csv('Calibrated J={:.3f} Bx={:.3f} dB={:.3f} BzMax={:.3f} dt={:.3f} gamma={:.3f}.csv'.format(J, Bx, dB,
                                                                                                           ...
                                                                                                           dt_steps,
                                                                                                           gamma))
        #df.to_csv('414\QubitTest\%i and %i.csv' %(pq0, pq1))
        #calibrated_df.to_csv('414\QubitTest\Calibrated %i and %i.csv' % (pq0, pq1))
        #print()
    else:
        df.to_csv(
...
def showfig(J, Bx, dB, Bz_max, dt_steps, dt, gamma, skip, provider, backend, pq0, pq1):
    ...
    #fig.write_html('414\QubitTest\%i and %i.html' % (pq0, pq1))
...
def main():
    ...
    #showfig(J, 0.05, dB, Bz_max, 1.6, dt, 4, 0, provider, backend, 2, 3)
    #showfig(J, 0.1, dB, Bz_max, 0.9, dt, 5, 0, provider, backend, 2, 3)



    #showfig(J, Bx, dB, Bz_max, dt_steps, dt, gamma, 0, provider, backend, 0, 1)
