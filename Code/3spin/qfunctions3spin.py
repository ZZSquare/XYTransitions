...
def XYMatrix(J):
    return J*(-0.25)*(np.kron(np.kron(I, X), X) + np.kron(np.kron(X, X), I) + np.kron(np.kron(I, Y), Y) + np.kron(np.kron(Y, Y), I))

def BxMatrix(Bx):
    return Bx*(np.kron(np.kron(X, I), I) + np.kron(np.kron(I, X), I) + np.kron(np.kron(I, I), X))

def ByMatrix(By):
    return By*(np.kron(np.kron(Y, I), I) + np.kron(np.kron(I, Y), I) + np.kron(np.kron(I, I), Y))

def BzMatrix(Bz):
    return Bz*(np.kron(np.kron(Z, I), I) + np.kron(np.kron(I, Z), I) + np.kron(np.kron(I, I), Z))
...
def adiabaticramp(J, Bx, dB, Bz_max, dt_steps, dt_steps_bool, g):
    ...
    if dt_steps_bool == 'dt':
        ...
        print("Number of steps: %f" %(time_steps_dt))
...
def exactm(J, Bx, Bzarray, dt):
    U = np.kron(np.kron(I, I), I)
    XY = XYMatrix(J)
    B_x = BxMatrix(Bx)
    psi0 = np.matrix([[0], [0], [0], [0], [0], [0], [0], [1]])
    mzarray = []
...
def Bxy_gate(Bx, theta, dt):
    circ = QuantumCircuit(3, name='exp{-i*Bxy*dt}')
    ...
    circ.i(0)
    circ.i(1)
    circ.u(2 * Bx * dt, theta + np.pi/2, -theta - np.pi/2, 2)
...
def threespin_instruction(circ, J, Bx, Theta, dt):
    for theta in Theta:
        #circ.barrier([0, 1, 2])
        circ.append(Hxy_gate(J, dt), [0, 1])
        circ.u(2 * Bx * dt, theta + np.pi / 2, -theta - np.pi / 2, 0)
        circ.append(Hxy_gate(J, dt), [1, 2])
        circ.u(2 * Bx * dt, theta + np.pi / 2, -theta - np.pi / 2, 1)
        #circ.append(Hxy_gate(J, dt), [2, 0])
        circ.u(2 * Bx * dt, theta + np.pi / 2, -theta - np.pi / 2, 2)
        #circ.barrier([0, 1, 2])
        #circ.u(2 * Bx * dt, theta + np.pi / 2, -theta - np.pi / 2, 0)
        #circ.i(1)
